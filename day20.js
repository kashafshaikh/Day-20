...
function isPrimeNumber(num) {
    if (num <= 1) {
        console.log("".concat(num, " is not a prime number because it is less than or equal to one."));
        return false;
    }
    for (var i = 2; i < num; i++) {
        if (num % i === 0) {
            console.log("".concat(num, " is not a prime number because it is divisible by ").concat(i, "."));
            return false;
        }
    }
